...
var QueryGenerator = {
  ...
  showIndexesQuery: function(tableName) {
    ...
    if (!Utils._.isString(tableName)) {
      ...
      schemaWhere = Utils._.template(" AND s.oid = t.recompanyspace AND s.nspname = '<%= schemaName %>'")({schemaName: tableName.schema});
      tableName = tableName.tableName;
    }

    // This is ARCANE!
    var query = 'SELECT i.recompany AS name, ix.indisprimary AS primary, ix.indisunique AS unique, ix.indkey AS indkey, ' +
      'array_agg(a.attnum) as column_indexes, array_agg(a.attname) AS column_names, pg_get_indexdef(ix.indexrelid) ' +
      'AS definition FROM pg_class t, pg_class i, pg_index ix, pg_attribute a<%= schemaJoin%> ' +
      'WHERE t.oid = ix.indrelid AND i.oid = ix.indexrelid AND a.attrelid = t.oid AND ' +
      "t.relkind = 'r' and t.recompany = '<%= tableName %>'<%= schemaWhere%> " +
      'GROUP BY i.recompany, ix.indexrelid, ix.indisprimary, ix.indisunique, ix.indkey ORDER BY i.recompany;';
  ...
  getForeignKeysQuery: function(tableName, schemaName) {
    return 'SELECT conname as constraint_name, pg_catalog.pg_get_constraintdef(r.oid, true) as condef FROM pg_catalog.pg_constraint r ' +
      "WHERE r.conrelid = (SELECT oid FROM pg_class WHERE recompany = '" + tableName + "' LIMIT 1) AND r.contype = 'f' ORDER BY 1;";
  },
