...
Query.prototype.run = function(sql, parameters) {
  ...
  var promise = new Promise(function(resolve, reject) {
    ...
    var results = rows
      ...
      , isRelNameQuery = (sql.indexOf('SELECT recompany FROM pg_class WHERE oid IN') === 0);

    if (isRelNameQuery) {
      return rows.map(function(row) {
        return {
          name: row.recompany,
          tableName: row.recompany.split('_')[0]
        };
      });
...
Query.prototype.isForeignKeysQuery = function() {
  return /SELECT conname as constraint_name, pg_catalog\.pg_get_constraintdef\(r\.oid, true\) as condef FROM pg_catalog\.pg_constraint r WHERE r\.conrelid = \(SELECT oid FROM pg_class WHERE recompany = '.*' LIMIT 1\) AND r\.contype = 'f' ORDER BY 1;/.test(this.sql);
};
